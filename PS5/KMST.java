    ...
    while (true) {
      ...
      for (int i = 0; i < m; i++) {
        ...
        if (!ufds.isSameSet(u, v)) {
          ...
          mstWeight += e.weight; // accumulate total weight so far
        }
      }
      ...
      if (!ufds.isSpanning()) {
        ...
        io.println(mstWeight); // print total weight
        io.flush();
        Collections.sort(mstEdges, new printedEdgeComparator()); // sort to lexicographic order, already sorted by weight order
